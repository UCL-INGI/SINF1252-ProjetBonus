...
// @get_bit:test_if_get_bit_return_zero => [La fonction get_bit ne retourne pas 0 lorsque la valeur du bit recherché est celle-ci]
void test_if_get_bit_return_zero(void)
{
	unsigned int binaryNumber = 0b10110010;
	unsigned char bitReturned = get_bit(binaryNumber,7);
	
	CU_ASSERT_EQUAL(bitReturned,0);
}

// @get_bit:test_if_get_bit_return_one => [La fonction get_bit ne retourne pas 1 lorsque la valeur du bit recherché est celle-ci]
void test_if_get_bit_return_one(void)
{
	unsigned int binaryNumber = 0b10110110;
	unsigned char bitReturned = get_bit(binaryNumber,3);
...
{
	unsigned int binaryNumber = 0b10110010;
	unsigned int binaryNumberReturned = set_bit(binaryNumber,2,0);
...
{
	unsigned int binaryNumber = 0b10110010;
	unsigned int binaryNumberReturned = set_bit(binaryNumber,7,1);
...
/* TESTS DE LA METHODE unset_first_bit */

// @unset_first_bit:test_unset_first_bit => [La fonction unset_first_bit ne ne met pas le premier bit trouvé dans une séquence passée en argument à 0]
void test_unset_first_bit(void)
{
	unsigned int binaryNumber = 0b1011011101;
	unsigned int binaryNumberReturned = unset_first_bit(binaryNumber);
...
int main(int argc, char* argv[])
{
	...
	if ( (NULL == CU_add_test(pSuite, "test_if_get_bit_return_zero", test_if_get_bit_return_zero)) ||
		 ...
		 (NULL == CU_add_test(pSuite, "test_unset_first_bit", test_unset_first_bit)) ||
